...
parser.add_argument("--post-optim", type=str, default=None, choices=[None,  'adam', 'lbfgs'])
parser.add_argument('--post-optim-mode', type=int, default=0)
parser.add_argument('--post-optim-lr', type=float, default=1)
parser.add_argument('--post-optim-epoch', type=int, default=1000)
parser.add_argument('--post-optim-lbfgs-iter', type=int, default=100)
...
args.post_optim_lr = args_new.post_optim_lr
args.post_optim_epoch = args_new.post_optim_epoch
args.post_optim_lbfgs_iter = args_new.post_optim_lbfgs_iter
args.rigid = args_new.rigid

args.n_iter = args_new.n_iter
args.pocket_prompt_nf = args_new.pocket_prompt_nf
args.complex_prompt_nf = args_new.complex_prompt_nf
...
if accelerator.is_main_process :
    ...
    '''
    metrics, _, _ = evaluate_mean_pocket_cls_coord_multi_task(accelerator, args, test_unseen_loader, accelerator.unwrap_model(model), com_coord_criterion, criterion, pocket_cls_criterion, pocket_coord_criterion, args.relative_k,
                                                              accelerator.device, pred_dis=pred_dis, use_y_mask=False, stage=2)
