...
class IaBNet_mean_and_pocket_prediction_cls_coords_dependent(torch.nn.Module):
    def __init__(self, args, embedding_channels=128, pocket_pred_embedding_channels=128):
        ...
        pocket_prompt_nf = args.pocket_prompt_nf
        complex_prompt_nf = args.complex_prompt_nf
        
        self.pocket_prompt_node_component = nn.Parameter(torch.zeros(1, pocket_prompt_nf, pocket_prompt_nf))
        self.pocket_prompt_coord_component = nn.Parameter(torch.zeros(1, pocket_prompt_nf, pocket_prompt_nf))

        self.complex_prompt_node_component = nn.Parameter(torch.zeros(1, complex_prompt_nf, complex_prompt_nf))
        self.complex_prompt_coord_component = nn.Parameter(torch.zeros(1, complex_prompt_nf, complex_prompt_nf))
        
        self.pocket_pred_model = EfficientMCAttModel(
            args, pocket_pred_embedding_channels, pocket_pred_embedding_channels, pocket_prompt_nf, n_channel, n_edge_feats=0, n_layers=args.pocket_pred_layers, n_iter=args.pocket_pred_n_iter,
        ...
        self.complex_model = EfficientMCAttModel(
            args, embedding_channels, embedding_channels, complex_prompt_nf, n_channel, n_edge_feats=0, n_layers=args.mean_layers, n_iter=args.n_iter,
            inter_cutoff=args.inter_cutoff, intra_cutoff=args.intra_cutoff, normalize_coord=self.normalize_coord, unnormalize_coord=self.unnormalize_coord,
        )
        ...
        #self.protein_bottleneck = nn.Linear(ankh_hidden, protein_hidden)
        ...
        #torch.nn.init.xavier_uniform_(self.protein_bottleneck.weight, gain=0.001)
    ...
    def forward(self, data, stage=1, train=False):
        ...
        complex_coords_whole_protein, complex_out_whole_protein, pocket_prompt_node_feat, pocket_prompt_coord_feat = self.pocket_pred_model(
            ...
            LAS_mask=None,
            prompt_node=self.pocket_prompt_node_component,
            prompt_coord=self.pocket_prompt_coord_component
        )
        ...
        complex_coords, complex_out, complex_prompt_node_feat, complex_prompt_coord_feat = self.complex_model(
            ...
            LAS_mask=None,
            prompt_node=self.complex_prompt_node_component,
            prompt_coord=self.complex_prompt_coord_component
        )
    ...
    def inference(self, data):
        ...
        return compound_coords_out, compound_batch, (pocket_prompt_node_feat, pocket_prompt_coord_feat), (complex_prompt_node_feat, complex_prompt_coord_feat)


def get_model(args, logger, device):
    if args.mode == 5:
        logger.log_message("PromptBind")
