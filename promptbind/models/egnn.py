...
class MC_E_GCL_Prompt(nn.Module):
    """
    Multi-Channel E(n) Equivariant Convolutional Layer
    """

    def __init__(self, args, input_nf, output_nf, hidden_nf, prompt_nf, n_channel, edges_in_d=0, act_fn=nn.SiLU(),
                 residual=True, attention=False, normalize=False, coords_agg='mean', tanh=False,
                 dropout=0.1, coord_change_maximum=10):
        super(MC_E_GCL_Prompt, self).__init__()
        input_edge = input_nf * 2
        self.args = args
        self.residual = residual
        self.attention = attention
        ...
        self.tanh = tanh
        self.epsilon = 1e-8
        self.prompt_nf = prompt_nf
        self.attn_nf = 4

        #######################################################        
        self.prompt_node_mlp_0 = nn.Sequential(
            nn.Linear(hidden_nf, prompt_nf),
            act_fn,
            nn.Linear(prompt_nf, prompt_nf**2),
            act_fn)
        self.prompt_node_mlp_1 = nn.Sequential(
            nn.Linear(prompt_nf**2, prompt_nf),
            act_fn,
            nn.Linear(prompt_nf, hidden_nf),
            act_fn)
        
        self.prompt_coord_mlp_0 = nn.Sequential(
            nn.Linear(hidden_nf, prompt_nf),
            act_fn,
            nn.Linear(prompt_nf, prompt_nf**2),
            act_fn)
        self.prompt_coord_mlp_1 = nn.Sequential(
            nn.Linear(prompt_nf**2, prompt_nf),
            act_fn,
            nn.Linear(prompt_nf, hidden_nf),
            act_fn)
        ...
        #######################################################
        self.prompt_node_mlp_agg_0 = nn.Linear(hidden_nf*2, hidden_nf)
        self.prompt_node_mlp_agg_1 = nn.Linear(self.attn_nf**2, hidden_nf)
        self.node_q, self.node_k, self.node_v = nn.Linear(hidden_nf, self.attn_nf**2), nn.Linear(hidden_nf, self.attn_nf**2), nn.Linear(hidden_nf, self.attn_nf**2)
        
        self.prompt_coord_mlp_agg_0 = nn.Linear(hidden_nf*2, hidden_nf)
        self.prompt_coord_mlp_agg_1 = nn.Linear(self.attn_nf**2, hidden_nf)
        self.coord_q, self.coord_k, self.coord_v = nn.Linear(hidden_nf, self.attn_nf**2), nn.Linear(hidden_nf, self.attn_nf**2), nn.Linear(hidden_nf, self.attn_nf**2)
    ...
    def node_model(self, x, edge_index, edge_attr_agg, node_attr):
        '''
        :param x: [bs * n_node, input_size]
        :param edge_index: list of [n_edge], [n_edge]
        :param edge_attr: [n_edge, hidden_size], refers to message from i to j
        ...
        '''
        row, col = edge_index
        #agg = unsorted_segment_sum(torch.cat([edge_attr, self.prompt_node_mlp(prompt)], dim=-1), row, num_segments=x.size(0))  # [bs * n_node, hidden_size]
        agg = unsorted_segment_sum(edge_attr_agg, row, num_segments=x.size(0))
        # print_log(f'agg1, {torch.isnan(agg).sum()}', level='DEBUG')
        if node_attr is not None:
            agg = torch.cat([x, agg, node_attr], dim=1)
        else:
    ...
    def coord_model(self, coord, edge_index, coord_diff, edge_feat_agg):
        '''
        coord: [bs * n_node, n_channel, d]
        edge_index: list of [n_edge], [n_edge]
        coord_diff: [n_edge, n_channel, d]
        ...
        '''
        row, col = edge_index
        n_edge, n_channel, d = coord_diff.size()
        #trans = coord_diff * self.coord_mlp(torch.cat([edge_feat, self.prompt_coord_mlp(prompt)], dim=1)).unsqueeze(-1) # [n_edge, n_channel, d]
        trans = coord_diff * self.coord_mlp(edge_feat_agg).unsqueeze(-1)
    ...
    def prompt_generation_module(self, edge_feat, prompt_node, prompt_coord) :        
        weight_feat_node = self.prompt_node_mlp_0(edge_feat)
        weight_feat_node = F.softmax(weight_feat_node.view(-1, self.prompt_nf, self.prompt_nf), dim=-1)
        prompt_node_feat = torch.flatten(weight_feat_node*prompt_node, start_dim=1)
        prompt_node_feat = self.prompt_node_mlp_1(prompt_node_feat)
        
        weight_feat_coord = self.prompt_coord_mlp_0(edge_feat)
        weight_feat_coord = F.softmax(weight_feat_coord.view(-1, self.prompt_nf, self.prompt_nf), dim=-1)
        prompt_coord_feat = torch.flatten(weight_feat_coord*prompt_coord, start_dim=1)
        prompt_coord_feat = self.prompt_coord_mlp_1(prompt_coord_feat)
                
        return prompt_node_feat, prompt_coord_feat # [n_edge, n_prompt]
    
    def prompt_interaction_module(self, edge_feat, prompt_node_feat, prompt_coord_feat) :
        prompt_node_feat_agg = self.prompt_node_mlp_agg_0(torch.cat([edge_feat, prompt_node_feat], dim=1))
        
        node_q, node_k, node_v = self.node_q(prompt_node_feat_agg), self.node_k(prompt_node_feat_agg), self.node_v(prompt_node_feat_agg)
        node_q, node_k, node_v = node_q.view(-1, self.attn_nf, self.attn_nf), node_k.view(-1, self.attn_nf, self.attn_nf), node_v.view(-1, self.attn_nf, self.attn_nf)
        
        node_weight = F.softmax(torch.bmm(node_q, node_k.permute(0, 2, 1)), dim=-1)
        node_residual = self.prompt_node_mlp_agg_1(torch.bmm(node_weight, node_v).view(-1, (self.attn_nf)**2))
        prompt_node_final = edge_feat + node_residual
        
        ###############################################################################################################################################
        
        prompt_coord_feat_agg = self.prompt_coord_mlp_agg_0(torch.cat([edge_feat, prompt_coord_feat], dim=1))
    
        coord_q, coord_k, coord_v = self.coord_q(prompt_coord_feat_agg), self.coord_k(prompt_coord_feat_agg), self.coord_v(prompt_coord_feat_agg)
        coord_q, coord_k, coord_v = coord_q.view(-1, self.attn_nf, self.attn_nf), coord_k.view(-1, self.attn_nf, self.attn_nf), coord_v.view(-1, self.attn_nf, self.attn_nf)
        
        coord_weight = F.softmax(torch.bmm(coord_q, coord_k.permute(0, 2, 1)), dim=-1)
        coord_residual = self.prompt_coord_mlp_agg_1(torch.bmm(coord_weight, coord_v).view(-1, (self.attn_nf)**2))
        prompt_coord_final = edge_feat + coord_residual
        
        return prompt_node_final, prompt_coord_final
        
    def forward(self, h, edge_index, coord, edge_attr=None, node_attr=None, batch_id=None, prompt_node=None, prompt_coord=None):
        '''
        h: [bs * n_node, hidden_size]
        edge_index: list of [n_row] and [n_col] where n_row == n_col (with no cutoff, n_row == bs * n_node * (n_node - 1))
        coord: [bs * n_node, n_channel, d]
        ...
        prompt_node_feat, prompt_coord_feat = self.prompt_generation_module(edge_feat, prompt_node, prompt_coord) # Newly Added
        edge_feat_node_agg, edge_feat_coord_agg = self.prompt_interaction_module(edge_feat, prompt_node_feat, prompt_coord_feat) # Newly Added
        
        # print_log(f'edge_feat, {torch.isnan(edge_feat).sum()}', level='DEBUG')
        coord = self.coord_model(coord, edge_index, coord_diff, edge_feat_coord_agg) # [bs * n_node, n_channel, d]
        h, agg = self.node_model(h, edge_index, edge_feat_node_agg, node_attr)
        
        return h, coord, edge_feat_node_agg, edge_feat_coord_agg
...
class MCAttEGNN(nn.Module):
    def __init__(self, args, in_node_nf, hidden_nf, out_node_nf, prompt_nf, n_channel, in_edge_nf=0,
        ...
        for i in range(0, n_layers):
            self.add_module(f'gcl_{i}', MC_E_GCL_Prompt(
                args, self.hidden_nf, self.hidden_nf, self.hidden_nf, prompt_nf, n_channel,
        ...
        #self.out_layer = MC_E_GCL(
            ...
            #coord_change_maximum=self.normalize_coord(10)
        #)
        self.out_layer = MC_E_GCL_Prompt(
            args, self.hidden_nf, self.hidden_nf, self.hidden_nf, prompt_nf, n_channel,
            edges_in_d=in_edge_nf, act_fn=act_fn, residual=residual,
            coord_change_maximum=self.normalize_coord(10)
        )
    ...
    def forward(self, h, x, ctx_edges, att_edges, LAS_edge_list, batched_complex_coord_LAS, 
                ...
                ctx_edge_attr=None, att_edge_attr=None, return_attention=False, prompt_node=None, prompt_coord=None):
        h = self.linear_in(h)
        h = self.dropout(h)
        x = x.clone()
        ...
        for i in range(0, self.n_layers):
            h, coord, prompt_node_feat, prompt_coord_feat = self._modules[f'gcl_{i}'](h, ctx_edges, x, batch_id=batch_id, edge_attr=ctx_edge_attr, prompt_node=prompt_node, prompt_coord=prompt_coord)
